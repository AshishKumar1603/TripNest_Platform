...
module.exports.index = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Listings fetch nahi ho payin" });
  }
};
...
module.exports.showListing = async (req, res) => {
  ...
  try {
    ...
    if (!listing) {
      return res
        .status(404)
        .json({ success: false, message: "Property nahi mili!" });
    }
...
// 3. CREATE LISTING (TripNest Style Geometry)
...
module.exports.filter = async (req, res) => {
  ...
  try {
    ...
    if (allListings.length !== 0) {
      ...
      res
        .status(404)
        .json({ success: false, message: `No listings found for ${id}` });
    }
  } catch (err) {
...
module.exports.search = async (req, res) => {
  ...
  if (!input) {
    return res
      .status(400)
      .json({ success: false, message: "Search value empty!" });
  }
  ...
  try {
    ...
    if (allListings.length !== 0) {
      ...
      res
        .status(404)
        .json({ success: false, message: "No matching results found" });
    }
  } catch (err) {
